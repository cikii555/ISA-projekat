...
public interface TransfusionCenterRepository extends JpaRepository<BloodTransfusionCenter,Integer> {
    @Query(value = "SELECT * FROM blood_transfusion_center b where LOWER(b.name) like LOWER(concat('%',:query,'%'))",nativeQuery = true)
    public ArrayList<BloodTransfusionCenter> searchBloodTransfusionCentersbyname(String query);

    @Query(value = "SELECT * FROM blood_transfusion_center b inner join address a ON b.address_id=a.id where LOWER(a.city) like LOWER(concat('%',:query,'%'))",nativeQuery = true)
